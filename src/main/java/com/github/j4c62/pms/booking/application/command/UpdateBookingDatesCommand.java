...
public record UpdateBookingDatesCommand(
    BookingId bookingId, BookingDates bookingDates, String updateReason) implements UpdateBookingCommand {
  @Override
  public BookingAggregate applyTo(BookingAggregate aggregate) {
    var event = new BookingUpdateEvent(bookingId, bookingDates, Instant.now());
